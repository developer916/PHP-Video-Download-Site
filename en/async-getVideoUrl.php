...
if(isset($_REQUEST['youtubeID'])) {
    ...
    if(strlen($my_id)>11){
        ...
        if( is_array($url) && count($url)>0 && isset($url['query']) && !empty($url['query']) ){
            ...
            if( !$my_id ){
                $data['result'] = "error";
                $data['message'] = $language_no_video_id_passed_in;
            ...
            $data['result'] = "error";
            $data['message'] = $language_invalid_url;
            header('Content-Type: application/json');
            echo json_encode($data);
            exit();
        }
...
if(isset($url_encoded_fmt_stream_map)) {
    ...
    $data['result'] = "error";
    $data['message'] = $language_no_encoded_format_stream_found."<br>".$language_here_is_what_we_got;
    header('Content-Type: application/json');
    echo json_encode($data);
    exit();
}
if (count($my_formats_array) == 0) {
    $data['result'] = "error";
    $data['message'] = $language_no_format_stream_map_found;
    header('Content-Type: application/json');
    echo json_encode($data);
    exit();
}
